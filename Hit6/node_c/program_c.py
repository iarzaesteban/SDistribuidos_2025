...
def register_with_d():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
            ...
            node_info = {"node_name": NODE_NAME, "ip": HOST, "port": PORT}
            s.sendall(json.dumps(node_info).encode())
            time.sleep(0.5)
            response = s.recv(1024).decode()
            logging.info(f"RECIBIMOPS LAS LISTAS DE NODOS C {json.loads(response)}")
            print(f"RECIBIMOPS LAS LISTAS DE NODOS C {json.loads(response)}")
