...
registry = []
# Lista para guardar las conexiones de los nodos C
connections = []
...
def handle_connection(conn, addr):
    ...
    if data:
        ...
        if node_info not in registry:
            ...
            logging.info(f"[NODE D] Vamos a registrar al nodo {node_info}")
            print(f"[NODE D] Vamos a registrar al nodo {node_info}")
...
def start_server():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        logging.info(f"[NODE D] Registro de contactos escuchando en el puerto {LISTEN_HOST}:{LISTEN_PORT}")
        print(f"[NODE D] Registro de contactos escuchando en el puerto {LISTEN_HOST}:{LISTEN_PORT}")
        while True:
